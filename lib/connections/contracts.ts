...
export function providerMetadata(provider: ConnectionProvider) {
  const metadata: Record<ConnectionProvider, { label: string; description: string; methods: readonly string[]; automated: boolean }> = {
    ...
    google_search_console: { label: "Google Search Console", description: "Read-only search queries, pages, clicks, and impressions.", methods: ["provider_invitation", "manual_inventory"], automated: false },
