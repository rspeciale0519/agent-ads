...
export default function AiReachChat({ organizationName, briefing }: Props) {
  ...
  return <div className="ai-reach-chat"><div className="ai-reach-chat-history" aria-live="polite"><div className="ai-reach-message ai-reach-message-assistant"><span className="ai-reach-message-label">AI Reach</span><p>{answer ?? "I will explain what the evidence supports and what still needs approval."}</p></div>{answer && <div className="ai-reach-message ai-reach-message-user"><span className="ai-reach-message-label">You</span><p>Question answered</p></div>}</div><div className="ai-reach-starters" aria-label="Starter questions">{starterQuestions.map((starter) => <button type="button" key={starter} onClick={() => ask(starter)}>{starter}</button>)}</div><form className="ai-reach-input" onSubmit={(event) => { event.preventDefault(); ask(question); }}><label htmlFor="ai-reach-question">Ask a question</label><div><input id="ai-reach-question" value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Example: What should I do first?" /><button className="primary-button" type="submit" disabled={!question.trim()}>Ask</button></div></form></div>;
}
