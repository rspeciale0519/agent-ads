...
export default async function AiReachPage() {
  ...
  const googleAdsTargets = (await Promise.all(dashboard.connections
    ...
    .map(async (connection): Promise<GoogleAdsReportTarget[]> => {
      const detail = await getConnectionDetail(context, connection.id);
      return detail.provider === "google_ads"
        ? detail.resources
          .filter((resource) => resource.resourceType === "ads_customer" && resource.selected && resource.eligibility === "eligible")
          .map((resource) => ({ connectionId: detail.id, customerId: resource.externalId, displayName: resource.displayName }))
        : [];
    }))).flat();
