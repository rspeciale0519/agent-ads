...
export function StepPanels({ form, activeStep, setField, toggleChannel, goTo, onFilesSelected, removeAttachment, uploadError }: Props) {
  return <div className="step-panels" data-active={activeStep}>
    <section className="step-panel" data-step="business">
      ...
      <div className="form-grid two-col">
        <Field label="Business name"><TextInput value={form.businessName} onChange={(value) => setField("businessName", value)} placeholder="e.g. MioDio HVAC" /></Field>
        <Field label="Website" hint="We’ll use it to understand your offer"><TextInput type="url" value={form.website} onChange={(value) => setField("website", value)} placeholder="https://yourcompany.com" /></Field>
      </div>
      <Field label="What do you sell and who is it for?" hint="Plain English works best"><TextArea value={form.description} onChange={(value) => setField("description", value)} placeholder="We help ..." /></Field>
      <div className="form-grid two-col">
        <Field label="Markets and service areas"><TextInput value={form.locations} onChange={(value) => setField("locations", value)} placeholder="Cities, regions, or worldwide" /></Field>
        <Field label="Business model"><select value={form.businessModel} onChange={(event) => setField("businessModel", event.target.value as FormData["businessModel"])}><option value="">Choose one</option><option value="B2B">B2B</option><option value="B2C">B2C</option><option value="B2B2C">B2B2C</option><option value="Other">Other</option></select></Field>
      </div>
    ...
    <section className="step-panel" data-step="goals">
      ...
      <div className="choice-grid goal-grid">{goalOptions.map((goal) => <ChoiceCard key={goal.value} selected={form.primaryGoal === goal.value} title={goal.title} description={goal.description} icon={goal.icon} onClick={() => setField("primaryGoal", goal.value)} />)}</div>
      <div className="form-grid two-col">
        <Field label="What are you promoting and who is the priority audience?"><TextArea value={form.goalDetails} onChange={(value) => setField("goalDetails", value)} placeholder="For example: book 30 more qualified demos from operations leaders without increasing our current cost per opportunity." rows={5} /></Field>
        <div className="stack-fields"><Field label="Approximate monthly media budget" hint="A range is fine"><TextInput value={form.monthlyBudget} onChange={(value) => setField("monthlyBudget", value)} placeholder="$5,000–$10,000" /></Field><Field label="What counts as a qualified lead or conversion?"><TextInput value={form.qualifiedOutcome} onChange={(value) => setField("qualifiedOutcome", value)} placeholder="e.g. booked demo with 20+ employee company" /></Field><Field label="Typical sales cycle"><TextInput value={form.salesCycle} onChange={(value) => setField("salesCycle", value)} placeholder="e.g. 14 days, or same-day purchase" /></Field></div>
      </div>
    </section>
    ...
    <section className="step-panel" data-step="brand">
      ...
      <Field label="Choose the closest brand voice"><div className="choice-grid voice-grid">{brandVoiceOptions.map((voice) => <ChoiceCard key={voice.value} selected={form.brandVoice === voice.value} title={voice.title} description={voice.description} icon={voice.icon} onClick={() => setField("brandVoice", voice.value)} />)}</div></Field>
      <div className="form-grid two-col"><Field label="Claims, topics, or language to avoid in ads and content"><TextArea value={form.prohibitedTopics} onChange={(value) => setField("prohibitedTopics", value)} placeholder="Anything sensitive, regulated, off-brand, or simply not you." /></Field><Field label="What creative assets already exist?"><TextArea value={form.existingAssets} onChange={(value) => setField("existingAssets", value)} placeholder="Brand guide, logos, product photos, customer stories, video library, etc." /></Field></div>
    ...
    <section className="step-panel" data-step="systems">
      ...
      <div className="form-grid two-col"><Field label="CRM or lead system"><select value={form.crm} onChange={(event) => setField("crm", event.target.value)}><option value="">Choose one</option><option>HubSpot</option><option>Salesforce</option><option>Pipedrive</option><option>Close</option><option>Other</option><option>None yet</option></select></Field><Field label="Web analytics"><select value={form.analytics} onChange={(event) => setField("analytics", event.target.value)}><option value="">Choose one</option><option>Google Analytics</option><option>PostHog</option><option>Segment</option><option>Other</option><option>Not sure</option></select></Field><Field label="Revenue or commerce source"><select value={form.revenueSource} onChange={(event) => setField("revenueSource", event.target.value)}><option value="">Choose one</option><option>Stripe</option><option>Shopify</option><option>QuickBooks</option><option>Our CRM</option><option>Other</option><option>Not tracked yet</option></select></Field><Field label="Who approves campaigns, creative, and budget changes?" hint="Names or roles"><TextInput value={form.teamApprovers} onChange={(value) => setField("teamApprovers", value)} placeholder="e.g. Maya (owner), Jordan (marketing)" /></Field></div>
      ...
      <Field label="Anything about your current marketing we should know?"><TextArea value={form.notes} onChange={(value) => setField("notes", value)} placeholder="Upcoming launches, seasonality, channel constraints, a recent win, or a problem you want us to diagnose first." rows={5} /></Field>
    </section>
