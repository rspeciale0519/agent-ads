...
import type { OnboardingValidationField, OnboardingValidationIssue } from "../../lib/onboarding-schema";
...
export function StepPanels({ form, activeStep, setField, toggleChannel, goTo, onFilesSelected, removeAttachment, uploadError, validationIssues }: Props) {
  ...
  return <div className="step-panels" data-active={activeStep}>
    <section className="step-panel" data-step="business">
      ...
      <Field label="What do you sell and who is it for?" fieldName="description" error={validationError("description")} hint="Plain English works best"><TextArea value={form.description} onChange={(value) => setField("description", value)} placeholder="We help ..." /></Field>
      <div className="form-grid two-col">
    ...
    <section className="step-panel" data-step="goals">
      ...
      <div className="form-grid two-col">
        <Field label="What are you promoting and who is the priority audience?" fieldName="goalDetails" error={validationError("goalDetails")}><TextArea value={form.goalDetails} onChange={(value) => setField("goalDetails", value)} placeholder="For example: book 30 more qualified demos from operations leaders without increasing our current cost per opportunity." rows={5} /></Field>
    ...
    <section className="step-panel" data-step="brand">
      ...
      <div className="form-grid two-col"><Field label="Claims, topics, or language to avoid in ads and content" fieldName="prohibitedTopics" error={validationError("prohibitedTopics")}><TextArea value={form.prohibitedTopics} onChange={(value) => setField("prohibitedTopics", value)} placeholder="Anything sensitive, regulated, off-brand, or simply not you." /></Field><Field label="What creative assets already exist?" fieldName="existingAssets" error={validationError("existingAssets")}><TextArea value={form.existingAssets} onChange={(value) => setField("existingAssets", value)} placeholder="Brand guide, logos, product photos, customer stories, video library, etc." /></Field></div>
