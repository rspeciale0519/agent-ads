...
- `POST /api/onboarding/submit` validates the full form with Zod, records a durable `onboarding_submissions` row, and sends a structured MioDio notification through Resend.
...
3. Copy the project URL, publishable key, and service-role key into the Vercel environment variables. The service-role key is server-only and must never be committed or prefixed with `NEXT_PUBLIC_`.
4. Keep the `onboarding-assets` bucket private. Uploaded files are untrusted business context and should be virus-scanned and reviewed before an agent uses them.
...
4. Deploy a Preview first, submit a test onboarding, upload an `.xlsx` and a PDF, and confirm both the Supabase row/files and Resend email.
5. Add the MioDio custom domain in Vercel after the preview test passes.
...
- Add an authenticated, expiring invitation/resume flow instead of leaving the public form anonymous.
